...
// create context
export const CSSQuizContext = createContext(initialState);

// create provider component
export const CSSQuizProvider = ({ children }) => {
      ...
      try {
        if (token) {
          ...
          dispatch({
            ...
            payload: res.data.cssAnsweredQuestions,
          });

          if (res.data.error) {
            setParentLoading(false);
            return setQuizError(res.data.error, 500);
          }
        } else {
          setParentLoading(false);
  ...
  const incrementScore = async () => {
    try {
      if (token) {
        const res = await axios({
          method: 'GET',
          url: '/api/questions/score/update',
          headers: {
            [tokenName]: token,
          },
        });
